...
object FieldChangeLimiter {
     ...
     * @param field The field that's being limited. For example, a user can only change their `USERNAME` once a month.
     */
    fun isLimited(type: String, id: Long, field: String): Pair<Boolean, Long?> {
        val find = Mongo.getClient()
            .getDatabase("global")
            .getCollection("namechange")
            .find(Filters.and(Filters.eq("id", id), Filters.eq("type", type), Filters.eq("field", field)))
            .firstOrNull()
    ...
    fun createLimit(type: String, id: Long, field: String, time: Long) {
        if (isLimited(type, id, field).first) {
            Mongo.getClient()
                ...
                .getCollection("namechange")
                .updateOne(
                    Filters.and(Filters.eq("id", id), Filters.eq("type", type), Filters.eq("field", field)),
