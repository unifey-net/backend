...
object CommunityInputRequirements : InputRequirements() {
    ...
    val DESCRIPTION: suspend (String) -> Unit = { desc: String ->
        if (desc.length > 256) {
            throw InvalidVariableInput(
                "Description", "That description is too long! Maximum is 256 characters.")
        }
    }
    ...
    val NAME: suspend (String) -> Unit = { name: String ->
        when {
            ...
            name.length > 16 ->
                throw InvalidVariableInput(
                    "Name", "That name is too long! Maximum is 16 characters.")
            3 > name.length ->
                throw InvalidVariableInput(
                    "Name", "That name is too short! Minimum is 3 characters.")
            !NAME_REGEX.matches(name) ->
                throw InvalidVariableInput(
                    "Name", "Please only use alphanumerics, - and _ in the name!")
        }
    }
}
