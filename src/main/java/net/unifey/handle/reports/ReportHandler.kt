...
object ReportHandler {
    ...
    fun getReportById(id: Long): Report {
        return reportCache.singleOrNull { report -> report.id == id }
    ...
    private val reportCache: MutableList<Report> by lazy {
        fun getTarget(document: Document): Target {
            return Target(
                    document.getLong("id"),
                    TargetType.valueOf(document.getString("type"))
            )
        }
    ...
    fun asReportRequest(reports: List<Report>): List<ReportRequest> {
        ...
        fun getUrlAndTarget(report: Report): Pair<String, String> {
            when (report.target.type) {
                TargetType.POST -> {
                    ...
                    return UserManager.getUser(post.authorId).username to "/${name.first}/${name.second}/${post.id}"
                }
                ...
                TargetType.COMMENT -> {
                    ...
                    return UserManager.getUser(comment.authorId).username to "/${name.first}/${name.second}/${comment.post}?comment=${comment.id}"
                }
        ...
        return reports.map { report ->
            ...
            ReportRequest(report, ReportData(
                    url,
                    getReportee(report),
                    target
            ))
