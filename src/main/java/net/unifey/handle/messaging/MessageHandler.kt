...
object MessageHandler {
    ...
    suspend fun sendSystemMessage(message: String, channel: Long) {
        val channelObject =
            when (ChannelHandler.getChannel<MessageChannel>(channel).type) {
        ...
        val messageObject =
            Message(
                Message.SYSTEM_ID, channelObject.id, message, System.currentTimeMillis(), listOf())

        val response =
            IncomingMessageResponse(
                channelObject, messageObject, Message.SYSTEM_ID to Message.SYSTEM_NAME)
    ...
    suspend fun sendMessage(user: Token, message: String, channel: Long) {
        ...
        val messageObject =
            Message(user.owner, channelObject.id, message, System.currentTimeMillis(), listOf())
        ...
        return MessageHistoryResponse(
            ...
            getMessages(channel, page).map { message ->
                if (message.user != Message.SYSTEM_ID) {
                    ...
                    IncomingMessageResponse(
                        channelObject, message, Message.SYSTEM_ID to Message.SYSTEM_NAME)
            })
    }
}
