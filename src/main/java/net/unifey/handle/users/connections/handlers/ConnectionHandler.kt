...
abstract class ConnectionHandler(rateLimit: Bandwidth) {
    private val rateLimitBucket: LocalBucket = Bucket4j.builder()
        .addLimit(rateLimit)
    ...
    suspend fun handleRateLimit() {
        ...
        LOGGER.trace("Checking rate limit for ${this.javaClass}: ${probe.isConsumed} -> ${probe.nanosToWaitForRefill} (${probe.remainingTokens})")
