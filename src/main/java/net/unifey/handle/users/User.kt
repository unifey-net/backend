...
class User(
    ...
    /** Join a [community] */
    fun join(community: Long) = member.join(community)

    /** Leave a [community] */
    fun leave(community: Long) = member.leave(community)
    ...
    val member by lazy {
        ...
        if (doc != null) {
            Member(
                id, doc["member"] as MutableList<Long>, doc["notifications"] as MutableList<Long>)
        } else {
            Mongo.getClient()
                .getDatabase("users")
                .getCollection("members")
    ...
    /** A user's friends. */
    fun getFriends() = FriendManager.getFriends(id)

    /** If the user is friends with [id] */
    fun hasFriend(id: Long) = getFriends().any { friend -> friend.id == id }
